...
def create_user_blueprint() -> Blueprint:
    ...
    bp.route('/bulk', methods=['POST'])(with_user_controller('bulk_create_users'))
    bp.route('/bulk/expiration', methods=['PUT'])(with_user_controller('bulk_update_expiration'))
    bp.route('/bulk/delete', methods=['DELETE'])(with_user_controller('bulk_delete_users'))
