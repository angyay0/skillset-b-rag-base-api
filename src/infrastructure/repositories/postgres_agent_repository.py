...
class PostgresAgentRepository(AgentRepository):
        ...
        for user_data in users_data:
            ...
            if db_user:
                ...
                if 'validity_days' in user_data and user_data['validity_days'] is not None:
                    ...
                    # Reset subscription_start_date to NOW when updating validity
                    db_user.subscription_start_date = now
                    
                    # Calculate subscription_end_date from NOW + validity_days
                    db_user.subscription_end_date = now + timedelta(days=user_data['validity_days'])
    ...
    def _user_to_dict(self, db_user: UserModel) -> dict:
        """Convert user model to dictionary with expiration info"""
        from datetime import datetime, timedelta, timezone
        
        # Calculate expiration date
        if db_user.subscription_end_date:
            expiration_date = db_user.subscription_end_date
        else:
            expiration_date = db_user.created_at + timedelta(days=db_user.validity_days)
        ...
        is_valid = current_date <= expiration_date and db_user.is_active
        ...
        return {
            ...
            'subscription_start_date': db_user.subscription_start_date.isoformat() if db_user.subscription_start_date else None,
            'subscription_end_date': db_user.subscription_end_date.isoformat() if db_user.subscription_end_date else expiration_date.isoformat(),
