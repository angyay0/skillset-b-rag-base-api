...
class VertexAIService:
    ...
    def generate_response(self, question: str, context: str = None, language: str = 'es', use_rag: bool = True, max_output_tokens: int = 110, custom_system_prompt: str = None, temperature: float = None) -> str:
        ...
        # Validate and cap max_output_tokens to prevent excessive token usage
        max_output_tokens = min(max(max_output_tokens, 50), 2048)  # Between 50 and 2048 tokens
        
        try:
            # Build prompt based on language with DP's personality
            system_prompts = {
                'es': '''Eres DP, un asistente amigable y servicial. Tu objetivo es ayudar a los usuarios de manera clara, concisa y amable. 
                ...
                'en': '''You are DP, a friendly and helpful assistant buddy. Your goal is to help users in a clear, concise, and kind way.
                Respond in English in a conversational and warm manner, like a good friend who wants to help.''',
                'pt': '''Você é DP, um assistente amigável e prestativo. Seu objetivo é ajudar os usuários de forma clara, concisa e gentil.
                Responda em português de maneira conversacional e calorosa, como um bom amigo que quer ajudar.'''
            }
            
            system_prompt = custom_system_prompt if custom_system_prompt else system_prompts.get(language, system_prompts['es'])
