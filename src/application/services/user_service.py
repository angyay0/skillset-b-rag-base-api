...
class UserService:
    ...
    def bulk_create_users(self, users_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with created, updated, and failed users
        """
        results = {
            'created': [],
            'updated': [],
        ...
        for user_data in users_data:
            try:
                ...
                if existing:
                    ...
                    updated_user = self._assign_existing_user_to_agents(existing, user_data)
                    results['updated'].append(self._to_response_dict(updated_user))
                else:
                    # Create new user
                    new_user = self.create_user(user_data)
                    results['created'].append(self._to_response_dict(new_user))
    ...
    def bulk_update_expiration(self, user_ids: List[int], validity_days: int) -> Dict[str, Any]:
        """
        Update expiration date for multiple users
        
        Args:
            user_ids: List of user IDs to update
            validity_days: Number of days to set as validity period
            
        Returns:
            Dictionary with updated and failed users
        """
        results = {
            'updated': [],
            'failed': []
        ...
        for user_id in user_ids:
            try:
                ...
                # Update validity_days and recalculate subscription_end_date
                user.validity_days = validity_days
                
                # Set subscription_start_date if not set
                if not user.subscription_start_date:
                    user.subscription_start_date = user.created_at
                
                # Calculate new subscription_end_date
                user.subscription_end_date = user.subscription_start_date + timedelta(days=validity_days)
                user.updated_at = now
    ...
    def bulk_delete_users(self, user_ids: List[int]) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dictionary with deleted and failed users
        """
        results = {
            'deleted': [],
            'failed': []
    ...
    def _assign_existing_user_to_agents(self, existing_user: User, user_data: Dict[str, Any]) -> User:
        ...
        # Update validity_days if provided and recalculate subscription_end_date
        if 'validity_days' in user_data:
            existing_user.validity_days = user_data['validity_days']
            # Recalculate subscription_end_date based on new validity_days
            if existing_user.subscription_start_date:
                existing_user.subscription_end_date = existing_user.subscription_start_date + timedelta(days=user_data['validity_days'])
            else:
                # If no subscription_start_date, set it to created_at and calculate end_date
                existing_user.subscription_start_date = existing_user.created_at
                existing_user.subscription_end_date = existing_user.created_at + timedelta(days=user_data['validity_days'])
            updated = True
        
        # Ensure subscription_end_date is set for existing users that don't have it
        if not existing_user.subscription_end_date:
            if not existing_user.subscription_start_date:
                existing_user.subscription_start_date = existing_user.created_at
            existing_user.subscription_end_date = existing_user.subscription_start_date + timedelta(days=existing_user.validity_days)
            updated = True
