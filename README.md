...
```bash
docker build -t dp-base-api .
docker run -p 5003:5003 --env-file .env dp-base-api
```
...
DATABASE_URL=postgresql://user:password@localhost:5432/dp_db
```
...
1. **Create Cloud SQL instance**:
   ```bash
   gcloud sql instances create dp-db \
...
2. **Create database**:
   ```bash
   gcloud sql databases create dp_db --instance=dp-db
   ```
...
4. **Set DATABASE_URL** for Cloud SQL:
   ...
   DATABASE_URL=postgresql://appuser:password@/dp_db?host=/cloudsql/PROJECT_ID:REGION:dp-db
   
   # For external connection
   DATABASE_URL=postgresql://appuser:password@INSTANCE_IP:5432/dp_db
   ```
